#!/usr/bin/env bash
# setup-gh-aliases.sh — Idempotent GitHub CLI alias configuration
# Works on macOS and Windows (Git Bash / WSL)
#
# Usage: bash scripts/setup-gh-aliases.sh
#
# Prerequisites: gh must be installed and authenticated (gh auth login)

set -euo pipefail

echo "Configuring gh CLI aliases..."
